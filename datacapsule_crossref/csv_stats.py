...
def calculate_counts_from_df_batches(df_batches, groupby_columns=None):
    def update_stats(stats, column):
        try:
            ...
            stats['count_valid'] = stats.get(
                'count_valid', 0) + len(column.dropna())
            ...
            if column_type != 'str':
                ...
                if len(numeric_values) > 0 and np.issubdtype(numeric_values.dtype, np.number):
                    ...
                    stats['count_numeric'] = stats.get(
                        'count_numeric', 0) + column_count_numeric
                    column_count_non_zero = (numeric_values != 0).sum()
                    stats['count_non_zero'] = stats.get(
                        'count_non_zero', 0) + column_count_non_zero
                    stats['count_zero'] = (
                        stats.get('count_zero', 0) +
                        column_count_numeric - column_count_non_zero
                    )
                elif len(column):
    ...
    for df_batch in df_batches:
        if groupby_columns:
            ...
            df_groups = split_and_drop_groupby_column(
                df_batch, groupby_columns)
        else:
...
def calculate_and_output_counts(argv):
    ...
    if args.header:
        ...
        del df_first
        result_column_names = (
            groupby_columns + ['stat'] +
            [s for s in column_names if s not in groupby_columns]
        )
        csv_writer.writerow(result_column_names)

    stats = calculate_counts_from_df_batches(
        df_batches, groupby_columns=groupby_columns)
